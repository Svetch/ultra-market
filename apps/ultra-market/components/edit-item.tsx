...
const EditableProductPage: React.FC<EditableProductPageProps> = ({
  product,
  onSave,
}) => {
    ...
    <form onSubmit={handleFormSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => setCategories(e.target.value.split(/,\s*/))}
          className="mt-1 p-2 border rounded w-full"
        />
      </div>
      ...
      <button
        type="submit"
        className="float-start px-4 py-2 bg-green-600 hover:bg-green-800 transition text-white rounded"
      >
        Változások mentése
      </button>
      <button
        onClick={() => setDialogVisible(true)}
        className="float-end px-4 py-2 bg-red-500 text-white rounded"
      >
        Árucikk törlése
      </button>
      <ConfirmationDialog
